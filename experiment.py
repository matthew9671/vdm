...
class Experiment(ABC):
  ...
  def train_and_evaluate(self, workdir: str):
    ...
    logging.info(f'Initial step: {initial_step}')
    ...
    with metric_writers.ensure_flushes(writer):
      ...
      while step < config.num_steps_train:
        ...
        if step % config.steps_per_logging == 0 or is_last_step:
          ...
          train_metrics = flax_utils.unreplicate(_train_metrics['scalars'])

          def avg_over_substeps(x):
            assert x.shape[0] == substeps
            return float(jnp.squeeze(x.mean(axis=0)))

          train_metrics = jax.tree_map(avg_over_substeps, train_metrics)
          if jax.process_index() == 0:
        ...
        if step % config.steps_per_eval == 0 or is_last_step:
  ...
  def evaluate(self, logdir, checkpoint_dir):
    ...
    tokens, samples = self.p_sample(params=params, rng=jax.random.split(self.rng, jax.local_device_count()))
